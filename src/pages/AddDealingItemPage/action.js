...
const handleSuccessNavigate = (navigate) => {
  navigate("/dealing-item-list");
};

export const postDealingItem = (payload, navigate) => async (dispatch) => {
  try {
    const response = await api.post(apiEndPoints.postDealingInItem(), payload, {
      headers,
    });
