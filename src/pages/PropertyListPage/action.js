import { toast } from "react-toastify";
import { api, apiEndPoints } from "../../api";
import { isNotthenSecondParameter } from "../../utils/helper";

export const GET_PROPERTY_PAGE = "GET_PROPERTY_PAGE";

const getPropertyDataSuccess = (payload) => {
  return {
    type: GET_PROPERTY_PAGE,
    payload: payload,
  };
};

export const getPropertyData = () => async (dispatch) => {
  try {
    const response = await api.get(apiEndPoints.getAllProperty());
    if (response?.data) {
      toast.success(response?.data?.message);
      console.log(response);
      dispatch(getPropertyDataSuccess(response));
    } else if (response?.response?.data?.message) {
      toast.error(response?.response?.data?.message);
    }
    console.log("response", response);
  } catch (error) {
    const { response: { data = {} } = {} } = error;
    return data;
  }
};

export const getSearchedPropertyData = (searchString) => async (dispatch) => {
  try {
    const response = await api.get(
      `property/search?query=${encodeURIComponent(searchString)}`
    );

    if (response?.data) {
      toast.success(response?.data?.message);
      console.log(response);
      dispatch(getPropertyDataSuccess(response));
    } else if (response?.response?.data?.message) {
      toast.error(response?.response?.data?.message);
    }
  } catch (error) {
    const { response: { data = {} } = {} } = error;
    return data;
  }
};

export const deleteProperty = (payload, navigate) => async (dispatch) => {
  try {
    console.log("ssss", payload._id);
    const response = await api.delete(
      apiEndPoints.deleteProperty(payload?._id)
    );
    if (response?.data) {
      toast.success(
        isNotthenSecondParameter(
          response?.data?.message,
          "Delete Data Successfull"
        )
      );
      window.location.reload();
    } else if (response?.response?.data?.message) {
      toast.error(response?.response?.data?.message);
    }
    console.log("response", response);
  } catch (error) {
    const { response: { data = {} } = {} } = error;
    return data;
  }
};
